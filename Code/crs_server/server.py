...
try:
    ...
    try:
        ...
        LOG.error(f"Cannot run server without global attributes file: {MASTER_PUBLIC_KEY_FILE}")
        LOG.error("Unexpected error:", exc_info()[0])
        exit()
...
try:
    ...
    try:
        ...
        LOG.error("FATAL ERROR: ABORTING SERVER")
        LOG.error(f"Cannot run server without global attributes file: \
...
def upload_file():
    if request.method == 'POST':
        ...
        if file.filename == '':
            ...
            return redirect(request.url)
        if file and allowed_file(file.filename, ENC_ALLOWED_EXTENSIONS):
...
def page_not_found(event):
    return render_template('404.html', event=event), 404
