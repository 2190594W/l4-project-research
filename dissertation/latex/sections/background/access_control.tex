...
Other Access Contol models do exist and generally all models fall into one of two categories, \textbf{capability-based} models or \textbf{access control lists-based} (ACL-based) models; where RBAC represents a capability-based model and ABE represents a ACL-based model.
\vskip 0.5em
\textit{Capability-based models} are based on the ability of a user to prove possession of an unforgeable (\todonote{\textit{Reference Wikipedia?}}) reference or \textit{capability} that aligns with the references of the system they are authorizing against. In the case of RBAC, the user proves that they possess the \textit{role} required for access via some immutable piece of data such as a cookie or JSON Web Token.\\
